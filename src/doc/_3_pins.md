...
for write access to files in the pinmuxing folders

~~~{.txt}
/sys/devices/ocp.?/pruio-??.??
~~~

for kernel 3.8 or for newer kernels

~~~{.txt}
/sys/devices/platform/ocp/ocp:pruio-??
~~~

There's one folder for each CPU ball, pre-defined in the device tree
overlay. By default the overlay claims only the free pins on the BBB
headers. Those are the orange and blue pins in the above image. When
you're using a Beaglebone White or Green, or when you free further pins
on the BBB, ie. by disabling HDMI or other features on the board, you
can easily create an universal overlay with adapted pin claiming by
using the tool dts_universal.bas.

If you don't want to execute the program under admin privileges, you
can configure the pins before you start the \Proj application (ie. in
the boot sequence). Just ensure that all used digital header pins are
in the appropriate mode before you start the program (so that the
initial checks succeed). This can get achieved by using the
`cape_universal` overlays and the tool `config_pin` shipped in the
default images, or by loading a customized overlay. In contrast to the
universal overlay, a customized contains only one configuration for
...
\note Digital output pins (GPIO, PWM) need no configuration, their
      state can get set at any time by the appropriate `setValue`
      function. In contrast input pins (GPIO, CAP, QEP) have to get
      configured by the appropriate `config` function at configuration
      time (before the call to function PruIo::config() ).

\note Some pins are used to control the boot sequence and mustn't be
      connected at boot-time, see \BbbRef{6.7} for details.
...
`libpruio-00A0.dtbo` for each claimed header pin. An input pin can get
configured with pullup or pulldown resistor, or none of them. Those
resistors (about 10 k) are incorporated in the CPU. In contrast, \Proj
configures an output pin always with no CPU resistor connection (to
minimize power consumption). So the first two modes (PRUIO_GPIO_OUT0
and PRUIO_GPIO_OUT1) use the same pinmuxing.

\note In some cases it may be reasonable to use an output pin with
      pullup resistor, which isn't supported by default. You can eithet
      use an external resistor or adapt the device tree overlay to use
...
| Pin   | Subsystem       | F_Min [Hz]           | F_Max [Hz] | Notice            |
| ----- | :-------------: | :------------------- | :--------- | :---------------- |
| P8_07 | TIMER-4         | 0.000021828 (12h43m) | 1.5e6 Hz   | free              |
| P8_08 | TIMER-7         | 0.000021828 (12h43m) | 1.5e6 Hz   | free              |
| P8_09 | TIMER-5         | 0.000021828 (12h43m) | 1.5e6 Hz   | free              |
| P8_10 | TIMER-6         | 0.000021828 (12h43m) | 1.5e6 Hz   | free              |
| P8_13 | PWMSS-2, PWM B  | 0.42 (2.38s)         | 50e6 Hz    | free              |
| P8_19 | PWMSS-2, PWM A  | 0.42 (2.38s)         | 50e6 Hz    | free              |
| P9_14 | PWMSS-1, PWM A  | 0.42 (2.38s)         | 50e6 Hz    | free              |
| P9_16 | PWMSS-1, PWM B  | 0.42 (2.38s)         | 50e6 Hz    | free              |
| P9_21 | PWMSS-0, PWM B  | 0.42 (2.38s)         | 50e6 Hz    | free              |
| P9_22 | PWMSS-0, PWM A  | 0.42 (2.38s)         | 50e6 Hz    | free              |
| P9_42 | PWMSS-0, CAP    | 0.0233 (42.9s)       | 50e6 Hz    | free (double pin) |
| P8_34 | PWMSS-1, PWM B  | 0.42 (2.38s)         | 50e6 Hz    | HDMI              |
| P8_36 | PWMSS-1, PWM A  | 0.42 (2.38s)         | 50e6 Hz    | HDMI              |
| P8_45 | PWMSS-2, PWM B  | 0.42 (2.38s)         | 50e6 Hz    | HDMI              |
| P8_46 | PWMSS-2, PWM A  | 0.42 (2.38s)         | 50e6 Hz    | HDMI              |
| P9_29 | PWMSS-0, PWM B  | 0.42 (2.38s)         | 50e6 Hz    | MCASP0            |
| P9_31 | PWMSS-0, PWM A  | 0.42 (2.38s)         | 50e6 Hz    | MCASP0            |
| P9_28 | PWMSS-2, CAP    | 0.0233 (42.9s)       | 50e6 Hz    | MCASP0            |
| JT_05 | PWMSS-2, CAP    | 0.0233 (42.9s)       | 50e6 Hz    | JTag (UART0_TXD)  |

The signal goes high at the beginning of the period and low when the
compare value is reached. The TIMER and PWMSS-CAP subsystems use a 32
bit counter and generate an independant pulse train. While the PWM-CAP
modules are clocked at 100 MHz (no pre-scaler), the TIMERs are clocked
at 24 MHz, resulting in a lower maximum frequency. In order to extended
the frequency range, the TIMERs counter clock can get pre-scaled, so
very long periods (low frequencies) are possible. The maximum is 12
hours and 43 minutes for TIMER subsystems (only 24 MHz clock input is
supported, see \ArmRef{20} for details).

In contrast the PWMSS-PWM subsystems use a 16 bit counter with 17 bit
duty resolution in up-down mode. The frequency range can also get
extended by a clock pre-scaler. \Proj auto-configures the mode and the
pre-scaler. For high frequencies (short counter periods) the counter
runs in up-count mode (16 bit resolution). For frequencies below 1526
Hz (= 100e6 / 65536) the counter runs in up-down mode (17 bit duty
resolution, 16 bit frequency resolution). In order to get the same
resolution for all settings, this up-down mode can be forced for high
frequencies also by masking the subsystems bit in member variable
...
case of six different events, see \ArmRef{15.2.4.3} for details. By
...
Additionally the PWMSS-PWM modules can get synchronized. Built-in
synchronization logic allows the time-base of multiple PWMSS-PWM
modules to work together as a single system. See \ArmRef{15.2.2.3} for
details.

All PWM outputs get specified by calling function PwmMod::setValue().
Since the output frequency may vary from the specified parameters due
to resolution issues, the real values can get computed by calling
function PwmMod::Value().
...
| Pin   | Subsystem      | F_Min [Hz]           | F_Max [Hz] | Notice            |
| ----- | :------------: | :------------------- | :----------| :---------------- |
| P8_07 | TIMER-4        | 0.000021828 (12h43m) | 1.e6 Hz    | free              |
| P9_28 | PWMSS-2, CAP   | 0.0233 (42.9s)       | 50e6 Hz    | MCASP0            |
| P9_42 | PWMSS-0, CAP   | 0.0233 (42.9s)       | 50e6 Hz    | free (double pin) |
| JT_04 | PWMSS-2, CAP   | 0.0233 (42.9s)       | 50e6 Hz    | JTag (UART0_RXD)  |

The measurement results get available by calling function
CapMod::Value(). An update is available right after the period
finished. Before using that function, you have to configure the pin for
CAP input by calling function CapMod::config() once.
