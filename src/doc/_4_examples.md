...
|                Name     |  Type  | Description          | src/examples     | src/c_examples | src/python      | libpruio-bin       |
| ----------------------: | :----- | :------------------- | :--------------- | :------------- | :-------------- | :----------------- |
| \ref sSecExaSimple      | IO:T   | Simple ADC input     | 1.bas            | 1.c            | 1.py            | pruio_1            |
| \ref sSecExaAnalyse     | IO:T   | Output system config | analyse.bas      |                |                 | pruio_analyse      |
| \ref sSecExaButton      | IO:T W | Simple Button        | button.bas       | button.c       | button.py       | pruio_button       |
| \ref sSecExaButton      | IO:TPW | Simple Button invers | button2.bas      | button2.c      | button2.py      | pruio_button2      |
| \ref sSecExaIoInput     | IO:T   | GPIO/ADC input       | io_input.bas     | io_input.c     | io_input.py     | pruio_io_input     |
| \ref sSecExaPerformance | IO:TPW | Pin toggling tests   | performance.bas  | performance.c  | performance.py  | pruio_performance  |
| \ref sSecExaPwmCap      | IO:TPW | CAP/PWM input/output | pwm_cap.bas      | pwm_cap.c      | pwm_cap.py      | pruio_pwm_cap      |
| \ref sSecExaPruAdd      |    T   | PRUSS firmware       | pruss_add.bas    | pruss_add.c    | pruss_add.py    | pruio_pruss_add    |
| \ref sSecExaPruToggle   | IO:TPW | GPIO->CAP with PRUSS | pruss_toggle.bas | pruss_toggle.c | pruss_toggle.py | pruio_pruss_toggle |
| \ref sSecExaQep         | IO:TPW | QEP input            | qep.bas          | qep.c          | qep.py          | pruio_qep          |
| \ref sSecExaRbFile      | RB:T   | Fast ADC file output | rb_file.bas      | rb_file.c      | rb_file.py      | pruio_rb_file      |
| \ref sSecExaSos         | IO:T   | User LED access      | sos.bas          | sos.c          | sos.py          | pruio_sos          |
| \ref sSecExaStepper     | IO:TPW | Uni-P stepper motor  | stepper.bas      | stepper.c      | stepper.py      | pruio_stepper      |
| \ref sSecExaPwmAdc      | IO:GPW | Compare PWM outputs  | pwm_adc.bas      |                |                 | pruio_pwm_adc      |
| \ref sSecExaOszi        | IO:G   | IO ADC input grafic  | oszi.bas         |                |                 | pruio_oszi         |
| \ref sSecExaRbOszi      | RB:G   | RB ADC input grafic  | rb_oszi.bas      |                |                 | pruio_rb_oszi      |
| \ref sSecExaTriggers    | MM:G W | RB ADC input grafic  | triggers.bas     |                |                 | pruio_triggers     |

Type: IO: = running in IO mode, RB: = running in RB mode, MM: = running
in MM mode, T = Text output, G = Graphic output, P = Pinmuxing
required, W = Wiring required.
