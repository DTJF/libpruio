...
FUNCTION TimerUdt.setValue CDECL( _
  ...
  STATIC AS CONST Float_t _
    d_min = 1000. *             &h4 / TMRSS_CLK _ '' minimal duration [mSec]
  , d_max = 1000. * &hFFFFFFFFFFuLL / TMRSS_CLK   '' maximal duration [mSec]
  DIM AS UInt32 nr = 0
  ...
  WITH *Top
    ...
    WITH *Conf(nr)
      ...
      CASE ELSE
        ...
        SELECT CASE AS CONST (cnt SHR 32) '' faster than LOG
        CASE   0        : .TCLR = TimMode
        ...
        IF Mode AND &b10 THEN ' one-shot
          IF dur < 0.01 THEN                     Top->Errr = E5 : RETURN Top->Errr ' one-shot duration out of range
          Raw(Nr)->CMax = 1
        ELSE
          Raw(Nr)->CMax = 0
        END IF
