...
#DEFINE ALL_START _
  "// dts file auto generated by libpruio tools (don't edit)" _
  !"\n/dts-v1/;" _
  !"\n/plugin/;" _
  !"\n" _
  !"\n/ {" _
  ...
  !"\n    manufacturer = ""TJF"";" _
  !"\n    part-number = ""PruIoDts"";" _
  !"\n    version = """ & VERS_NAME & """;" _
  !"\n" _
  !"\n    // state the resources this overlay uses" _
  !"\n    exclusive-use ="
...
#MACRO CREATE()
  ...
  ELSE
    ...
    IF LEN(COMMAND(1)) THEN TARG_PATH = COMMAND(1) '    compile with dtc
...
ENUM PinModes
  ...
  I_O = RX + NP '*< setting for an in-/out-put periperal pin.
  IOD = RX + NP '*< setting for an in-/out-put digital bus pin.
...
#DEFINE GPIO_DEF _
  ...
  , 7 + NP        ' output (no resistor)

'* The default settings for PRU GPIO input lines.
#DEFINE PRUI_DEF(_M_) _
    _M_ + RX + NP _ ' input, open (no resistor)
  , _M_ + RX + PU _ ' input, pullup resistor
  , _M_ + RX + PD   ' input, pulldown resistor
...
'* A variable to add user fragments (starting at fragment@3 {}), designed to enable subsystems.
DIM SHARED AS STRING USER_FRAGS
